...
if menu == "🏠 Home":
    ...
    - **✂️ Split Data**  
      Memisahkan data menjadi data latih (*training set*), data validasi (*validation set*) dan data uji (*testing set*).  
    ...
    try:
        with open(pickle_path, "rb") as f:
            preproc_artifact = pickle.load(f)
    except Exception as e:
    ...
    try:
        fig_px = px.line(
            ...
            x="tanggal",
            y="jumlah",
            title=f"Jumlah Kunjungan Wisatawan — {wisata_choice}",
            markers=True,
            template="plotly_white",
            labels={ "tanggal":"Bulan", "jumlah":"Jumlah Wisatawan" }
        )
        fig_px.update_yaxes(tickformat=",")
        idx_max = df["jumlah"].idxmax()
        x_max = df.loc[idx_max, "tanggal"]
        y_max = df.loc[idx_max, "jumlah"]
        fig_px.add_scatter(x=[x_max], y=[y_max], mode="markers+text",
                           text=[f"{int(y_max):,}".replace(",", ".")],
    ...
    try:
        with open(pickle_path, "rb") as f:
            ceemdan_artifact = pickle.load(f)
    except Exception as e:
    ...
    try:
        ...
        fig_orig = px.line(df_orig, y="value", title=f"Sinyal Asli — {jumlah_col} — {wisata_choice}", labels={"value":"Jumlah Wisatawan", "index":"Index Waktu"})

        max_idx = int(df_orig["value"].idxmax())
        fig_orig.add_trace(go.Scatter(x=[max_idx], y=[df_orig.loc[max_idx,"value"]], mode="markers+text",
                                     text=[f"{int(df_orig.loc[max_idx,'value']):,}".replace(",", ".")], textposition="top center",
                                     marker=dict(size=10)))
    ...
    if not sel_components:
        ...
        try:
            ...
            fig = make_subplots(rows=rows, cols=1, shared_xaxes=True, vertical_spacing=0.02,
                                subplot_titles=sel_components[:rows])

            for i, name in enumerate(sel_components[:rows]):
                y = np.asarray(components[name], dtype=float)
                fig.add_trace(go.Scatter(y=y, mode="lines+markers", name=name, marker=dict(size=4)), row=i+1, col=1)
                energy = imf_energy.get(name, None)
                if energy is not None:
                    fig.add_annotation(xref="paper", yref="paper",
                                       x=0.99, y=(1 - (i / rows)) - 0.01,
                                       text=f"E={energy:.2f}%", showarrow=False, font=dict(size=10, color="#444"))
            fig.update_layout(height=230 * rows, showlegend=False, title_text=f"IMF terpilih ({len(sel_components)} komponen)")
    ...
    if not sel_components:
        ...
        for comp in sel_components:
            ...
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=df_plot["index"], y=df_plot["normalized"], mode="lines+markers", name=f"{comp} (Normalized)", marker=dict(size=4)))
            if "inverse" in df_plot:
                fig.add_trace(go.Scatter(x=df_plot["index"], y=df_plot["inverse"], mode="lines", name=f"{comp} (Inverse → Skala Asli)", line=dict(dash="dash")))

            fig.update_layout(
                height=320,
                margin=dict(l=40, r=20, t=50, b=40),
                legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
                title=f"{comp} — Normalized vs Inverse"
            )
    ...
    split_files = {
        "Wisata Brumbun": {
            "80_10_10": "wisata_brumbun/split_result_80_10_10.pkl",
            "90_5_5": "wisata_brumbun/split_result_90_5_5.pkl",
        },
        "Wisata Goa Maharani": {
            "80_10_10": "goa_maharani/split_result_80_10_10.pkl",
            "90_5_5": "goa_maharani/split_result_90_5_5.pkl",
        },
        "Wisata Makam Sendang Duwur": {
            "80_10_10": "makam_sendang_duwur/split_result_80_10_10.pkl",
            "90_5_5": "makam_sendang_duwur/split_result_90_5_5.pkl",
        },
        "Wisata Makam Sunan Drajat": {
            "80_10_10": "makam_sunan_drajat/split_result_80_10_10.pkl",
            "90_5_5": "makam_sunan_drajat/split_result_90_5_5.pkl",
        },
        "Wisata Monumen Van Der Wijk": {
            "80_10_10": "monumen_van_der_wijk/split_result_80_10_10.pkl",
            "90_5_5": "monumen_van_der_wijk/split_result_90_5_5.pkl",
        },
        "Wisata Museum Sunan Drajat": {
            "80_10_10": "museum_sunan_drajat/split_result_80_10_10.pkl",
            "90_5_5": "museum_sunan_drajat/split_result_90_5_5.pkl",
        },
        "Wisata Waduk Gondang": {
            "80_10_10": "waduk_gondang/split_result_80_10_10.pkl",
            "90_5_5": "waduk_gondang/split_result_90_5_5.pkl",
        },
        "Wisata Bahari Lamongan": {
            "80_10_10": "wisata_bahari_lamongan/split_result_80_10_10.pkl",
            "90_5_5": "wisata_bahari_lamongan/split_result_90_5_5.pkl",
        },
    }

    col_wisata, col_split = st.columns([2, 1])
    with col_wisata:
        wisata_choice = st.selectbox("🗺 Pilih Data Wisata:", list(split_files.keys()))
    with col_split:
        split_label_ui = st.radio(
            "Pilih rasio split:",
            ["80% / 10% / 10%", "90% / 5% / 5%"],
            horizontal=True
        )

    split_key = "80_10_10" if "80%" in split_label_ui else "90_5_5"
    pickle_path = split_files[wisata_choice][split_key]

    st.info(f"📂 File split yang akan dimuat: `{pickle_path}`")

    if not os.path.exists(pickle_path):
        st.error(f"❌ File `{pickle_path}` tidak ditemukan.")
        st.stop()

    with open(pickle_path, "rb") as f:
        split_artifact = pickle.load(f)

    train_components = split_artifact.get("train_components", {})
    val_components   = split_artifact.get("val_components", {})
    test_components  = split_artifact.get("test_components", {})

    y_train = np.asarray(split_artifact.get("y_train", []), dtype=float)
    y_val   = np.asarray(split_artifact.get("y_val", []), dtype=float)
    ...
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        st.metric("Total (N)", len(y_train) + len(y_val) + len(y_test))
    with c2:
        st.metric("Latih", len(y_train))
    with c3:
        st.metric("Validasi", len(y_val))
    with c4:
        st.metric("Uji", len(y_test))

    st.write(f"Label split: **{label}**")
    st.markdown("---")

    st.subheader("🧩 Panjang Komponen IMF (Latih / Validasi / Uji)")
    comp_names = list(train_components.keys())

    rows = []
    for name in comp_names[:6]:
        rows.append({
            "Komponen": name,
            "Latih": len(train_components.get(name, [])),
            "Validasi": len(val_components.get(name, [])),
            "Uji": len(test_components.get(name, []))
        })

    st.dataframe(pd.DataFrame(rows), use_container_width=True)

    st.markdown("---")
    st.subheader("📈 Visualisasi Sinyal Asli (Latih + Validasi + Uji)")

    full_series = np.concatenate([y_train, y_val, y_test])
    df_plot = pd.DataFrame({
        "index": np.arange(len(full_series)),
        "value": full_series,
        "set": (
            ["Latih"] * len(y_train)
            + ["Validasi"] * len(y_val)
    ...
    fig = px.line(
        ...
        color="set",
        markers=True,
        title=f"Sinyal Asli — {wisata_choice} — Split {label}",
        labels={"index": "Index Waktu", "value": "Jumlah Wisatawan", "set": "Subset"}
    )

    if len(y_train) > 0:
        fig.add_vline(x=len(y_train)-1, line_dash="dash", annotation_text="Latih → Validasi")
    if len(y_train) + len(y_val) > 0:
        fig.add_vline(x=len(y_train)+len(y_val)-1, line_dash="dash", annotation_text="Validasi → Uji")
    ...
    st.markdown("---")
    st.success("🎉 Split data (Latih–Validasi–Uji) berhasil ditampilkan dan siap digunakan ke tahap modelling.")

elif menu == "📊 Modelling":
    st.markdown("---")
    st.markdown(
        """
        <div style="font-size:26px;font-weight:700;display:flex;gap:10px">
            <span>📊</span><span>Menu Modelling</span>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    st.write("""
    Pada tahap ini ditampilkan **ringkasan hasil modelling terbaru**:
    - **CEEMDAN–ELM (tanpa PSO)**
    - **CEEMDAN–ELM–PSO**
    
    Evaluasi dilakukan pada **data latih, validasi, dan uji**.
    """)

    model_files = {
        "Wisata Brumbun": {
            "elm": {
                "80_10_10": "wisata_brumbun/modelling_elm_split_80_10_10.pkl",
                "90_5_5": "wisata_brumbun/modelling_elm_split_90_5_5.pkl",
            },
            "pso": {
                ("80_10_10", "cfg1"): "wisata_brumbun/modelling_elm_pso_split_80_10_10_cfg1.pkl",
                ("80_10_10", "cfg2"): "wisata_brumbun/modelling_elm_pso_split_80_10_10_cfg2.pkl",
                ("90_5_5", "cfg1"): "wisata_brumbun/modelling_elm_pso_split_90_5_5_cfg1.pkl",
                ("90_5_5", "cfg2"): "wisata_brumbun/modelling_elm_pso_split_90_5_5_cfg2.pkl",
            }
        },
        "Wisata Goa Maharani": {
            "elm": {
                "80_10_10": "goa_maharani/modelling_elm_split_80_10_10.pkl",
                "90_5_5": "goa_maharani/modelling_elm_split_90_5_5.pkl",
            },
            "pso": {
                ("80_10_10", "cfg1"): "goa_maharani/modelling_elm_pso_split_80_10_10_cfg1.pkl",
                ("80_10_10", "cfg2"): "goa_maharani/modelling_elm_pso_split_80_10_10_cfg2.pkl",
                ("90_5_5", "cfg1"): "goa_maharani/modelling_elm_pso_split_90_5_5_cfg1.pkl",
                ("90_5_5", "cfg2"): "goa_maharani/modelling_elm_pso_split_90_5_5_cfg2.pkl",
            }
        },
        "Wisata Makam Sendang Duwur": {
            "elm": {
                "80_10_10": "makam_sendang_duwur/modelling_elm_split_80_10_10.pkl",
                "90_5_5": "makam_sendang_duwur/modelling_elm_split_90_5_5.pkl",
            },
            "pso": {
                ("80_10_10", "cfg1"): "makam_sendang_duwur/modelling_elm_pso_split_80_10_10_cfg1.pkl",
                ("80_10_10", "cfg2"): "makam_sendang_duwur/modelling_elm_pso_split_80_10_10_cfg2.pkl",
                ("90_5_5", "cfg1"): "makam_sendang_duwur/modelling_elm_pso_split_90_5_5_cfg1.pkl",
                ("90_5_5", "cfg2"): "makam_sendang_duwur/modelling_elm_pso_split_90_5_5_cfg2.pkl",
            }
        },
        "Wisata Makam Sunan Drajat": {
            "elm": {
                "80_10_10": "makam_sunan_drajat/modelling_elm_split_80_10_10.pkl",
                "90_5_5": "makam_sunan_drajat/modelling_elm_split_90_5_5.pkl",
            },
            "pso": {
                ("80_10_10", "cfg1"): "makam_sunan_drajat/modelling_elm_pso_split_80_10_10_cfg1.pkl",
                ("80_10_10", "cfg2"): "makam_sunan_drajat/modelling_elm_pso_split_80_10_10_cfg2.pkl",
                ("90_5_5", "cfg1"): "makam_sunan_drajat/modelling_elm_pso_split_90_5_5_cfg1.pkl",
                ("90_5_5", "cfg2"): "makam_sunan_drajat/modelling_elm_pso_split_90_5_5_cfg2.pkl",
            }
        },
        "Wisata Monumen Van Der Wijk": {
            "elm": {
                "80_10_10": "monumen_van_der_wijk/modelling_elm_split_80_10_10.pkl",
                "90_5_5": "monumen_van_der_wijk/modelling_elm_split_90_5_5.pkl",
            },
            "pso": {
                ("80_10_10", "cfg1"): "monumen_van_der_wijk/modelling_elm_pso_split_80_10_10_cfg1.pkl",
                ("80_10_10", "cfg2"): "monumen_van_der_wijk/modelling_elm_pso_split_80_10_10_cfg2.pkl",
                ("90_5_5", "cfg1"): "monumen_van_der_wijk/modelling_elm_pso_split_90_5_5_cfg1.pkl",
                ("90_5_5", "cfg2"): "monumen_van_der_wijk/modelling_elm_pso_split_90_5_5_cfg2.pkl",
            }
        },
        "Wisata Museum Sunan Drajat": {
            "elm": {
                "80_10_10": "museum_sunan_drajat/modelling_elm_split_80_10_10.pkl",
                "90_5_5": "museum_sunan_drajat/modelling_elm_split_90_5_5.pkl",
            },
            "pso": {
                ("80_10_10", "cfg1"): "museum_sunan_drajat/modelling_elm_pso_split_80_10_10_cfg1.pkl",
                ("80_10_10", "cfg2"): "museum_sunan_drajat/modelling_elm_pso_split_80_10_10_cfg2.pkl",
                ("90_5_5", "cfg1"): "museum_sunan_drajat/modelling_elm_pso_split_90_5_5_cfg1.pkl",
                ("90_5_5", "cfg2"): "museum_sunan_drajat/modelling_elm_pso_split_90_5_5_cfg2.pkl",
            }
        },
        "Wisata Waduk Gondang": {
            "elm": {
                "80_10_10": "waduk_gondang/modelling_elm_split_80_10_10.pkl",
                "90_5_5": "waduk_gondang/modelling_elm_split_90_5_5.pkl",
            },
            "pso": {
                ("80_10_10", "cfg1"): "waduk_gondang/modelling_elm_pso_split_80_10_10_cfg1.pkl",
                ("80_10_10", "cfg2"): "waduk_gondang/modelling_elm_pso_split_80_10_10_cfg2.pkl",
                ("90_5_5", "cfg1"): "waduk_gondang/modelling_elm_pso_split_90_5_5_cfg1.pkl",
                ("90_5_5", "cfg2"): "waduk_gondang/modelling_elm_pso_split_90_5_5_cfg2.pkl",
            }
        },
        "Wisata Bahari Lamongan": {
            "elm": {
                "80_10_10": "wisata_bahari_lamongan/modelling_elm_split_80_10_10.pkl",
                "90_5_5": "wisata_bahari_lamongan/modelling_elm_split_90_5_5.pkl",
            },
            "pso": {
                ("80_10_10", "cfg1"): "wisata_bahari_lamongan/modelling_elm_pso_split_80_10_10_cfg1.pkl",
                ("80_10_10", "cfg2"): "wisata_bahari_lamongan/modelling_elm_pso_split_80_10_10_cfg2.pkl",
                ("90_5_5", "cfg1"): "wisata_bahari_lamongan/modelling_elm_pso_split_90_5_5_cfg1.pkl",
                ("90_5_5", "cfg2"): "wisata_bahari_lamongan/modelling_elm_pso_split_90_5_5_cfg2.pkl",
            }
        }
    }

    wisata_choice = st.selectbox("🗺 Pilih Data Wisata:", list(model_files.keys()))
    st.markdown(f"**Wisata terpilih:** {wisata_choice}")

    rows_elm, rows_pso = [], []

    for split_key, file_path in model_files[wisata_choice]["elm"].items():
        ...
        m = art["mape"]
        rows_elm.append({
            "Split": split_key.replace("_", "/"),
            "MAPE Train (%)": m["train"] * 100,
            "MAPE Validasi (%)": m["val"] * 100,
            "MAPE Test (%)": m["test"] * 100,
        })

    for (split_key, cfg), file_path in model_files[wisata_choice]["pso"].items():
        if not os.path.exists(file_path):
            continue
    ...
    if not rows_pso:
        st.error("❌ Tidak ada file modelling yang berhasil dimuat.")
        st.stop()
    ...
    tab1, tab2 = st.tabs(["📄 CEEMDAN–ELM", "🚀 CEEMDAN–ELM–PSO"])

    with tab1:
        st.subheader("📊 Hasil CEEMDAN–ELM (Tanpa PSO)")
        st.dataframe(
            df_elm.style.format({
                "MAPE Train (%)": "{:.2f}",
                "MAPE Validasi (%)": "{:.2f}",
    ...
    with tab2:
        st.subheader("📊 Hasil CEEMDAN–ELM–PSO")
        st.dataframe(
            df_pso.style.format({
                "MAPE Train (%)": "{:.2f}",
                "MAPE Validasi (%)": "{:.2f}",
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.metric("MAPE Test Terbaik (%)", f"{best_row['MAPE Test (%)']:.2f}")
        st.metric("Split", best_row["Split"])
    with c2:
        st.metric("Konfigurasi", best_row["Konfigurasi"])
